...
def read_results(noise_file_path, denoise_file_path):
    # original_results = pd.read_csv(original_file_path)
    noise_results = pd.read_csv(noise_file_path)
    denoise_results = pd.read_csv(denoise_file_path)
    selected_columns = ['dataset', 'base_learner', 'noise_ratio', 
    ...
    noise_results = noise_results[selected_columns]
    denoise_results = denoise_results[selected_columns]
    noise_results['adaptation_f1_all_macro_mean'] = noise_results['adaptation_f1_all_macro_mean'].round(4)
    noise_results['adaptation_f1_all_macro_std'] = noise_results['adaptation_f1_all_macro_std'].round(4)
    noise_results['adaptation_f1_new_macro_mean'] = noise_results['adaptation_f1_new_macro_mean'].round(4)
    noise_results['adaptation_f1_new_macro_std'] = noise_results['adaptation_f1_new_macro_std'].round(4)
    noise_results['adaptation_f1_old_macro_mean'] = noise_results['adaptation_f1_old_macro_mean'].round(4)
    noise_results['adaptation_f1_old_macro_std'] = noise_results['adaptation_f1_old_macro_std'].round(4)
    denoise_results['adaptation_f1_all_macro_mean'] = denoise_results['adaptation_f1_all_macro_mean'].round(4)
    denoise_results['adaptation_f1_all_macro_std'] = denoise_results['adaptation_f1_all_macro_std'].round(4)
    denoise_results['adaptation_f1_new_macro_mean'] = denoise_results['adaptation_f1_new_macro_mean'].round(4)
    denoise_results['adaptation_f1_new_macro_std'] = denoise_results['adaptation_f1_new_macro_std'].round(4)
    denoise_results['adaptation_f1_old_macro_mean'] = denoise_results['adaptation_f1_old_macro_mean'].round(4)
    denoise_results['adaptation_f1_old_macro_std'] = denoise_results['adaptation_f1_old_macro_std'].round(4)
    return noise_results, denoise_results

def map_data_to_dict(noise_results, denoise_results):
    dataset_list = ['cic2018', 'edge_iiot', 'iot_nid']
    NN_results = {dataset: {'Add_noise': {}, 'Denoise_LOF': {}} for dataset in dataset_list}
    LR_results = {dataset: {'Add_noise': {}, 'Denoise_LOF': {}} for dataset in dataset_list}
    result_dict = {'NN': NN_results, 'LR': LR_results}

    # Find NN
    noise_results_NN = noise_results.groupby('base_learner').get_group('nn')
    denoise_results_NN = denoise_results.groupby('base_learner').get_group('nn')
    for dataname in dataset_list:
        noise_results_NN_dataname = noise_results_NN.groupby('dataset').get_group(dataname).sort_values(by='noise_ratio')
        denoise_results_NN_dataname = denoise_results_NN.groupby('dataset').get_group(dataname).sort_values(by='noise_ratio')
        result_dict['NN'][dataname]['noise_ratio'] = noise_results_NN_dataname['noise_ratio'].tolist()
        result_dict['NN'][dataname]['Add_noise'] = {
            'F1-ALL': [noise_results_NN_dataname['adaptation_f1_all_macro_mean'].tolist(), noise_results_NN_dataname['adaptation_f1_all_macro_std'].tolist()],
            'F1-New': [noise_results_NN_dataname['adaptation_f1_new_macro_mean'].tolist(), noise_results_NN_dataname['adaptation_f1_new_macro_std'].tolist()],
            'F1-Old': [noise_results_NN_dataname['adaptation_f1_old_macro_mean'].tolist(), noise_results_NN_dataname['adaptation_f1_old_macro_std'].tolist()],
        }
        result_dict['NN'][dataname]['Denoise_LOF'] = {
            'F1-ALL': [denoise_results_NN_dataname['adaptation_f1_all_macro_mean'].tolist(), denoise_results_NN_dataname['adaptation_f1_all_macro_std'].tolist()],
            'F1-New': [denoise_results_NN_dataname['adaptation_f1_new_macro_mean'].tolist(), denoise_results_NN_dataname['adaptation_f1_new_macro_std'].tolist()],
            'F1-Old': [denoise_results_NN_dataname['adaptation_f1_old_macro_mean'].tolist(), denoise_results_NN_dataname['adaptation_f1_old_macro_std'].tolist()],
        }

    # Find LR
    noise_results_LR = noise_results.groupby('base_learner').get_group('lr')
    denoise_results_LR = denoise_results.groupby('base_learner').get_group('lr')
    for dataname in dataset_list:
        noise_results_LR_dataname = noise_results_LR.groupby('dataset').get_group(dataname).sort_values(by='noise_ratio')
        denoise_results_LR_dataname = denoise_results_LR.groupby('dataset').get_group(dataname).sort_values(by='noise_ratio')
        result_dict['LR'][dataname]['noise_ratio'] = noise_results_LR_dataname['noise_ratio'].tolist()
        result_dict['LR'][dataname]['Add_noise'] = {
            'F1-ALL': [noise_results_LR_dataname['adaptation_f1_all_macro_mean'].tolist(), noise_results_LR_dataname['adaptation_f1_all_macro_std'].tolist()],
            'F1-New': [noise_results_LR_dataname['adaptation_f1_new_macro_mean'].tolist(), noise_results_LR_dataname['adaptation_f1_new_macro_std'].tolist()],
            'F1-Old': [noise_results_LR_dataname['adaptation_f1_old_macro_mean'].tolist(), noise_results_LR_dataname['adaptation_f1_old_macro_std'].tolist()],
        }
        result_dict['LR'][dataname]['Denoise_LOF'] = {
            'F1-ALL': [denoise_results_LR_dataname['adaptation_f1_all_macro_mean'].tolist(), denoise_results_LR_dataname['adaptation_f1_all_macro_std'].tolist()],
            'F1-New': [denoise_results_LR_dataname['adaptation_f1_new_macro_mean'].tolist(), denoise_results_LR_dataname['adaptation_f1_new_macro_std'].tolist()],
            'F1-Old': [denoise_results_LR_dataname['adaptation_f1_old_macro_mean'].tolist(), denoise_results_LR_dataname['adaptation_f1_old_macro_std'].tolist()],
        }
    pprint.pprint(result_dict, compact=True)
    return result_dict


    #     for index, row in noise_results_NN_dataname.iterrows():
    #         noise_ratio = row['noise_ratio']
    #         adaptation_f1_all_macro_mean = row['adaptation_f1_all_macro_mean']
    #         adaptation_f1_all_macro_std = row['adaptation_f1_all_macro_std']
    # print(noise_results_NN)
    # print(denoise_results_NN)
    # for index, row in noise_results.iterrows():
    # for index, row in noise_results_NN.iterrows():
    #     dataset = row['dataset']
    #     noise_ratio = row['noise_ratio']
    #     adaptation_f1_all_macro_mean = row['adaptation_f1_all_macro_mean']
    #     adaptation_f1_all_macro_std = row['adaptation_f1_all_macro_std']
    #     adaptation_f1_new_macro_mean = row['adaptation_f1_new_macro_mean']
    # return result_dict
...
def _plot_grid_for_classifier(data, clf, figsize=(14, 12), show_std=False):
    """
    Plot one 3x3 figure for a classifier.
    Rows  : datasets [cic2018, edge_iiot, iot_nid]
    Cols  : metrics  [F1-ALL, F1-New, F1-Old]
    Lines : Add_noise vs Denoise_LOF
    """
    ...
    fig.suptitle(f'{clf} — Noise Sensitivity (Add_noise vs Denoise_LOF)', y=0.92, fontsize=14)
    ...
    for r, ds in enumerate(datasets):
        ...
        for c, metric in enumerate(metrics):
            ...
            # 两条线：Add_noise 与 Denoise_LOF
            add_mean = data[clf][ds]['Add_noise'][key][0]
            den_mean = data[clf][ds]['Denoise_LOF'][key][0]

            ax.plot(noise_ratio, add_mean, marker='o', label='Add_noise')
            ax.plot(noise_ratio, den_mean, marker='^', label='Denoise_LOF')

            if show_std:
                add_std = data[clf][ds]['Add_noise'][key][1]
                den_std = data[clf][ds]['Denoise_LOF'][key][1]
                ax.fill_between(noise_ratio,
                                [m-s for m, s in zip(add_mean, add_std)],
                                [m+s for m, s in zip(add_mean, add_std)],
                                alpha=0.15)
                ax.fill_between(noise_ratio,
                                [m-s for m, s in zip(den_mean, den_std)],
                                [m+s for m, s in zip(den_mean, den_std)],
                                alpha=0.15)
            ...
            # if r == 0 and c == 2:
            ax.legend(loc='lower left')  # 只在右上角的子图放图例，避免重复

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    # plt.show()
    plt.savefig(f'./PDF/{clf}.pdf', bbox_inches='tight', facecolor='white', dpi=300)

def plot_nn_lr_3x3(data, show_std=False):
    # """生成两张图：第一张 NN（3x3），第二张 LR（3x3）。"""
    _plot_grid_for_classifier(data, 'NN',  figsize=(12, 6), show_std=show_std)
    _plot_grid_for_classifier(data, 'LR',  figsize=(12, 6), show_std=show_std)

if __name__ == "__main__":
    noise_file_path = "../metrics/bestcombo_student_metrics_noise_new.csv"
    denoise_file_path = "../metrics/bestcombo_student_metrics_ProtoMargin_denoise.csv"

    noise_results, denoise_results = read_results(noise_file_path, denoise_file_path)

    print(noise_results.head())
    print(denoise_results.head())
    print(noise_results.columns)
    # exit()
    result_dict = map_data_to_dict(noise_results, denoise_results)
